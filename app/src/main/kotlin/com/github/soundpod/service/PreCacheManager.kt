...
class PreCacheManager(
    ...
    private suspend fun preCacheSong(videoId: String) {
        if (cacheManager.isCached(videoId, 0, 512 * 1024L)) {
            Log.i("GhostKernel-PreCache", "$videoId is already in cache, skipping.")
            return
        }

        Log.d("GhostKernel-PreCache", "⏳ Pre-caching $videoId...")
        ...
        if (uri == null) {
            println("GhostKernel-PreCache: Innertube failed for $videoId, falling back to NewPipe")
            runCatching {
                val extractor = ServiceList.YouTube.getStreamExtractor("https://www.youtube.com/watch?v=$videoId")
                extractor.fetchPage()
                println("GhostAudioSource(PreCache): found ${extractor.audioStreams.size} streams for $videoId")
                val bestAudio = extractor.audioStreams
                    .filter { it.codec?.lowercase()?.contains("opus") == true }
                    .maxByOrNull { it.averageBitrate }
                    ?: extractor.audioStreams.maxByOrNull { it.averageBitrate }
                
                uri = bestAudio?.content?.toUri()
                println("GhostAudioSource(PreCache): selected codec=${bestAudio?.codec} avgBitrate=${bestAudio?.averageBitrate}")
                userAgent = null
            }.onFailure { e ->
                println("GhostKernel-PreCache: NewPipe fallback failed for $videoId: ${e.message}")
            }
        }
        ...
        if (uri != null) {
            ...
            Log.w("GhostKernel-PreCache", "No direct URL found for $videoId after fallback")
            return
        }
        ...
        try {
            ...
            Log.i("GhostKernel-PreCache", "Successfully buffered 256KB for $videoId")
        } catch (e: Exception) {
            Log.e("GhostKernel-PreCache", "Caching failed for $videoId: ${e.message}")
        }
    }
    ...
    fun cleanUp() {
        scope.launch {
            ...
            oldSongs.forEach { song ->
                ...
                Log.d("GhostKernel-PreCache", "Cleaned up expired pre-cache for ${song.id}")
            }
        }
    }
}
