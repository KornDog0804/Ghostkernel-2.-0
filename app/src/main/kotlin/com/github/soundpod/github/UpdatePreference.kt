...
fun downloadApk(context: Context, url: String): Long {
    val request = DownloadManager.Request(url.toUri())
        ...
        .setDescription("Downloading latest GhostKernel APK…")
        .setNotificationVisibility(DownloadManager.Request.VISIBILITY_VISIBLE_NOTIFY_COMPLETED)
        .setDestinationInExternalPublicDir(
            Environment.DIRECTORY_DOWNLOADS,
            "GhostKernel-latest.apk"
        )
...
suspend fun checkForUpdates(
    ...
    try {
        ...
        if (latestVersion != null && apkAsset != null) {
            ...
            if (isNew) {
                val fileName = "GhostKernel_v$latestVersion.apk"
...
suspend fun downloadViaDownloadManager(
    ...
    try {
        val request = DownloadManager.Request(urlString.toUri())
            .setTitle("GhostKernel Update")
            .setDescription("Downloading latest version...")
            .setNotificationVisibility(DownloadManager.Request.VISIBILITY_VISIBLE_NOTIFY_COMPLETED)
            .setDestinationInExternalPublicDir(
                Environment.DIRECTORY_DOWNLOADS,
                "GhostKernel-Update.apk"
            )
...
fun openPublicFile(context: Context, file: File) {
    try {
        ...
        try {
            ...
            Toast.makeText(context, "Saved to Downloads: GhostKernel-Update.apk", Toast.LENGTH_LONG)
                .show()
        }
    }
}
