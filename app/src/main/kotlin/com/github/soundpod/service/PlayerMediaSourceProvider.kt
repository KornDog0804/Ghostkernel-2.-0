        ...
        lock.withLock {
            ...
            val fastResult: Pair<Uri, String?>? = runCatching {
                if (!YouTubeSessionManager.isSessionReady.value) {
                    ...
                    Log.d("GhostKernel", "Session ready, resolving $videoId")
                }
                val result = runBlocking { Innertube.player(videoId)?.getOrNull() }
                if (result == null) {
                    Log.w("GhostKernel", "Innertube.player($videoId) returned null result")
                }
                val format = result?.response?.streamingData?.highestQualityFormat
                if (result != null && format == null) {
                    Log.w("GhostKernel", "Innertube.player($videoId) succeeded but highestQualityFormat is null")
                }
                val uri = format?.url?.toUri()
                if (format != null && uri == null) {
                    Log.w("GhostKernel", "Innertube.player($videoId) format found (itag=${format.itag}) but url is null - may need signature decipher")
                }
                uri?.let { it to result?.userAgent }
            }.onFailure { e ->
                Log.e("GhostKernel", "Fast path (Innertube.player) threw for $videoId", e)
            }.getOrNull()
            ...
            val rawUrl = runCatching<String> {
                ...
                println("GhostAudioSource: NewPipeExtractor found ${audioStreams.size} audio streams for $videoId:")
                audioStreams.forEach {
                    println("GhostAudioSource:   codec=${it.codec} avgBitrate=${it.averageBitrate}")
                }

                val bestAudio: YoutubeStreamExtractor.Stream = audioStreams
                    .filter { it.codec?.lowercase(Locale.ROOT)?.contains("opus") == true }
                    .maxByOrNull { it.averageBitrate }
                    ?: audioStreams.maxByOrNull { it.averageBitrate }
                    ?: throw Exception("No audio-only streams found")

                println("GhostAudioSource: Selected codec=${bestAudio.codec} avgBitrate=${bestAudio.averageBitrate}")

                bestAudio.content
            }.getOrElse { e ->
                Log.e("GhostKernel", "Resolution failed for $videoId", e)
                throw e
            }
...
private class YouTubeErrorPolicy(
    ...
    override fun getRetryDelayMsFor(loadErrorInfo: LoadErrorHandlingPolicy.LoadErrorInfo): Long {
        ...
        if (isTransient(exception) && videoId != null) {
            Log.w("GhostKernel", "Retrying $videoId (attempt $retryCount, transient error: $exception)")
        ...
        if (videoId != null) {
            Log.e("GhostKernel", "Fatal error for $videoId (count $retryCount): $exception")
            urlCache.remove(videoId)
        }
