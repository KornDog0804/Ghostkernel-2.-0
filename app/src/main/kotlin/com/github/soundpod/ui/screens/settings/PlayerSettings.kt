...
import com.github.soundpod.utils.dnaEqEnabledKey
...
fun PlayerSettingsContent(
    ...
    var loudnessBoost by rememberPreference(loudnessBoostKey, 0)
    var dnaEqEnabled by rememberPreference(dnaEqEnabledKey, false)
    ...
    SettingsGroup(
        ...
        SwitchSetting(
            icon = IconSource.Vector(Icons.Default.MusicOff),
            title = "DNA Mood EQ",
            description = "Subtle EQ shift based on your Music DNA moods. Off by default - try it and see how it feels.",
            switchState = dnaEqEnabled,
            onSwitchChange = {
                dnaEqEnabled = it
            }
        )
        SliderSettingsItem(
            label = "Extra Boost",
            value = loudnessBoost.toFloat(),
            onValueChange = {
                loudnessBoost = it.toInt()
                binder?.audioEffectManager?.setLoudnessBoost(it.toInt())
            },
            valueRange = 0f..1000f,
            valueLabel = { String.format(Locale.US, "%.2fx", 1.2f + (it / 1000f) * 0.5f) },
            defaultValue = 0f,
            hapticUseIntegerStep = true,
            hapticUseFloatStep = false,
            hapticFloatStep = 100f
        )
    }
