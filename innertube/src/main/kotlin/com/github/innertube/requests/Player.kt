...
suspend fun Innertube.player(videoId: String): Result<PlayerResult>? = runCatchingNonCancellable {
    ...
    if (currentContext != null) {
        println("Innertube: DYNAMIC context being sent: client=${currentContext.client}")
        println("Innertube: poToken at request time: ${poToken?.take(20)}... (length=${poToken?.length ?: 0})")
        ...
        if (dynamicResponse?.playabilityStatus?.status == "OK") {
            ...
            println("Innertube: DYNAMIC client failed for $videoId: status=${dynamicResponse?.playabilityStatus?.status} reason=${dynamicResponse?.playabilityStatus?.reason} messages=${dynamicResponse?.playabilityStatus?.messages}")
        }
    }
