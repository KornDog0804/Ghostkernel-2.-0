...
class QuickPicksViewModel : ViewModel() {
    ...
    fun loadQuickPicks(quickPicksSource: QuickPicksSource, forceRefresh: Boolean = false) {
        ...
        job = viewModelScope.launch(Dispatchers.IO) {
            Log.d("GhostKernel", "Loading Quick Picks for source: $quickPicksSource")
            val seedSongs = runCatching {
                when (quickPicksSource) {
                    ...
                    QuickPicksSource.Trending -> {
                        // Ghost Picks: use GhostKernel's own listening history first.
                        // Fall back to YouTube charts only when the local brain has no plays yet.
                        val ghostSeeds = getSeedSongsFlow(quickPicksSource, 3).first()
                        ghostSeeds.ifEmpty {
                ...
                Log.e("GhostKernel", "Failed to load seed songs for $quickPicksSource", e)
                emptyList()
            }

            Log.d("GhostKernel", "Seed songs found: ${seedSongs.size}")
            ...
            coroutineScope {
                ...
                Log.d("GhostKernel", "Related results found: ${relatedResults.size}")
                ...
                if (mergedPage == null || mergedPage.songs.isNullOrEmpty()) {
                    Log.d("GhostKernel", "No related songs, trying charts fallback")
                ...
                if (mergedPage == null || mergedPage.songs.isNullOrEmpty()) {
                    Log.d("GhostKernel", "Still no songs, trying global fallbacks")
